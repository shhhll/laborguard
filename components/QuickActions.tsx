'use client';
  ...
  import { Image, FileText, MessageSquare, ChevronRight, Shield } from               
  'lucide-react';                                                                    
  ...
  export default function QuickActions() {                  
    return (                              
      <section>
        <h2 className="text-sm font-semibold text-gray-500 mb-3 px-0.5">빠른
  메뉴</h2>                                                                          
        <div className="grid grid-cols-2 gap-3">
          {ACTIONS.map(({ icon: Icon, color, iconColor, title, desc, badge }) => (   
            <button                                                                  
              key={title}
              className="card text-left hover:shadow-card-hover active:scale-[0.98]  
  transition-all duration-150 flex flex-col gap-3"                                   
            >                                                                        
              <div className="flex items-start justify-between">                     
                <div className={`w-9 h-9 rounded-xl ${color} flex items-center       
  justify-center`}>                                                                  
                  <Icon className={iconColor} style={{ width: 18, height: 18 }} />
                </div>                                                               
                {badge && (                                                          
                  <span className="text-[10px] font-bold bg-accent text-white px-2 
  py-0.5 rounded-full">                                                              
                    {badge}                                                          
                  </span>  
                )}                                                                   
